...
func handleCreateConnection(s *server) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if in.Name == "" || in.Host == "" || in.Port == 0 || in.Username == "" {
			writeError(w, r, http.StatusBadRequest, CodeInvalidInput, "name, host, port, username are required")
			return
		}
...
func handleRevealPassword(s *server) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		// Defer zero AFTER writing — copy to local string first.
		creds.Zero()
